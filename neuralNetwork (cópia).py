...
if __name__ == "__main__":
    ...
    for fold in range(0,k):
        ...
        inputs = np.array([[matrixProcessed[y,0],matrix[y,1],matrixProcessed[y,2]]  for y in treinoIndex])        
        outputs = np.array([int(matrix[y,4] == 1) for y in treinoIndex],dtype="int32")        
        print(int(np.max(inputs[:,1]))+1)        
        ...
        print(idadeDescretizada)
        inputs = np.array([[inputs[y,0],idadeDescretizada[y],inputs[y,2]] for y in range(len(inputs[:,0]))])
       
        model = theanets.Classifier([3, (7, 'tanh'), 2],loss='CrossEntropy')   
       
        model.train([inputs, outputs],algo='nag',learning_rate=0.01,momentum=0.9)
        ...
        testInputs = np.array([[matrixProcessed[y,0],matrix[y,1],matrixProcessed[y,2]]  for y in testeIndex]) 
        testIdadeDescretizada  = min_max_scaler.fit_transform(infartoIdade[np.array(testInputs[:,1],dtype="int32"),1])
        
        predicts = model.predict(np.array([[testInputs[y,0],idadeDescretizada[y],testInputs[y,2]] for y in range(len(testInputs[:,0]))]))
