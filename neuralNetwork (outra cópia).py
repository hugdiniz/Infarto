...
if __name__ == "__main__":
    ...
    for fold in range(0,k):
        ...
        arrayPredicts = np.array([])
        boostingNumber = 1
        boostingLimiar = 0
        neuralNumber = [7,7,8,8,9,10,11,12,13,14,15]
        for kBoosting in range(boostingNumber):
            ...
            model = theanets.Classifier([3, (7, 'tanh'),(7, 'tanh'), 2],loss='CrossEntropy',weighted=True)   
        ...
        #    print(tm['loss'])
            ...
            if(len(arrayPredicts) == 0):
                arrayPredicts = np.array([predicts])
            else:
                arrayPredicts = np.array(np.concatenate((arrayPredicts, [predicts]), axis=0))

        testOuts = np.array([int(matrix[y,4] == 1) for y in testeIndex],dtype="int32")
        print(arrayPredicts)
        predicts = [int(arrayPredicts[:,x].sum() > boostingLimiar) for x in range(len(arrayPredicts[0,:]))]
